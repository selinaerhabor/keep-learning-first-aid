...
# kidsquestions = {
#     'What phone number should you call in an emergency?':['(a)0800','(b)100','(c)999'],
#     'Who can give first aid treatment?':['(a)Only ambulance staff','(b)Only adults','(c)Adults and children'],
#     'If someone is choking, how can you help?':['(a)Hit them on the back','(b)Lean them backwards','(c)Lie them on their side'],
#     'What is the best thing to put on a burn at first?':['(a)Warm running water','(b)Cold running water','(c)Kitchen film'],
#     'Pressing on the wound is the treatment for what?':['(a)A broken bone','(b)A bad bleed','(c)A bee sting'],
#     'Which of these is the best way to treat a nose bleed?':['(a)Lean head forward, pinch soft part of the nose','(b)Lean head forward, pinch hard part of the nose','(c)Turn them on their side, with their head forward'],
#     'Why might someone use an inhaler?':['(a)They get nosebleeds a lot','(b)They have asthma','(c)They fall over often'],
#     'If you find someone collapsed on the floor, what should you do first?':['(a)Put my jacket over them to keep them warm','(b)Check if they are breathing','(c)Turn them on their side, with their head forward'],
#     'How would you help an unconscious person who is breathing?':['(a)Sit them up and make sure their head is facing forward','(b)Turn them on their side, with their head back','(c)Turn them on their side, with their head forward'],
#     'What is the best way to help a person with a broken leg?':['(a)Press on it','(b)Pour cold water on it','(c)Keep it still and support it']
# }

# questions = copy.deepcopy(kidsquestions)

# @app.route('/kidsquiz')
# def quiz():
#  for i in questions.keys():
#   random.shuffle(questions[i])
#  return render_template('learningforkidsquiz.html')
...
#  i = 0
#  while i < len(sq):
#   current_answer = random.choice(shuffled_questions.keys())
#   if current_answer not in user_answers:
...
# @app.route('/kidsquiz/end', methods=['POST'])
# def quiz_answers():
#    correct = 0
#    for i in questions.keys():
